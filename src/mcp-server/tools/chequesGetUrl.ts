...
const args = {
  request: operations.ChequesGetURLRequest$inboundSchema,
};

export const tool$chequesGetUrl: ToolDefinition<typeof args> = {
  name: "cheques-get-url",
  description: `Get Cheque Preview
...
than the regular order retrieval endpoint, so it is suitable for customer-facing use-cases.`,
  ...
  tool: async (client, args, ctx) => {
    const [result, apiCall] = await chequesGetUrl(
      client,
      args.request,
