...
export class Boxes extends APIResource {
   ...
   * @example
   * ```ts
   * const box = await client.printMail.v1.boxes.create({
   *   cheques: [
   *     {
   *       from: 'contact_456',
   *       to: 'contact_123',
   ...
   * @example
   * ```ts
   * const box = await client.printMail.v1.boxes.retrieve('id');
   * ```
   */
   ...
   * for await (const boxListResponse of client.printMail.v1.boxes.list()) {
   *   // ...
   * }
   * ```
   */
  list(
    ...
    return this._client.getAPIList('/print-mail/v1/boxes', List<BoxListResponse>, { query, ...options });
  }
   ...
   * @example
   * ```ts
   * const box = await client.printMail.v1.boxes.delete('id');
   * ```
   */
